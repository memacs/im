#!/usr/bin/env bash
# Release 冒烟：健康检查 + 容器内消息/会话未读（见 release-smoke-messaging.md）
set -euo pipefail
...
if ! kubectl -n "$NAMESPACE" get deployment im &>/dev/null; then
  log "No deployment/im in $NAMESPACE — skip in-pod messaging smoke"
  exit 0
fi

log "In-pod messaging smoke (bin/smoke-messaging)..."
kubectl -n "$NAMESPACE" exec deployment/im -- /app/bin/smoke-messaging

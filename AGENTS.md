...
**用户要求「讲解 / 分析 / 详细解释」某段代码、流程、架构、设计时，AI 必须：**

1. **先在对话中交付完整讲解正文**（满足用户当前提问）；
2. **随后立刻把同一份讲解内容整理成独立 Markdown 文档**，落在仓库文档目录，**不得只留在对话上下文里**——否则下次会话/协作者无法引用。

### 2.1 文档落位与命名

| 内容类型 | 落位目录 | 命名规则 | 示例 |
| --- | --- | --- | --- |
| **Elixir 实现**层面（模块、热路径、状态机、监督树、单模块跨文件流程） | `docs/implementation/elixir/` | `模块名或主题.md`，用 kebab-case，对应源码文件名或主题 | `outbound-queue-scheduling.md`（对应 `outbound_queue.ex` + `packet_transport.ex`）、`cid-dedup-gateway.md`（对应 `cid_dedup.ex`） |
| **设计/协议**层面（跨语言、选型理由、为什么） | `docs/design/` | 同上 | `why-drf-wfq.md`、`transport-binary-frame.md` |
| **Java 实现**层面 | `docs/implementation/java/` | 同上 | `dispatch-router-design.md` |
| **Web/前端**层面 | `docs/implementation/web/` | 同上 | `console-proto-adapter.md` |
| **运维 / K8s / 部署**层面 | `docs/implementation/elixir/`（K8s 与 IM Release 部署放这）已有 `deploy-guide.md`、`flamegraph.md` 等就沿用；新专题再新文件 | 同上 | `postgresql-port-routing.md` |

**命名原则**：文件名应能让第三方一眼看出主题，**避免用 `explain-*` `analysis-*` `note-*` 这种无信息量前缀**。直接写主题名。

### 2.2 文档最低结构

讲解文档必须包含（至少 3 项，按文档类型调整）：

- **标题**：一句话说明"这篇讲什么 + 对应哪些源码文件"
- **总体架构 / 角色关系**：谁调谁、进程/模块边界
- **核心流程拆解**：按阶段/步骤 + 对应源码行号 + GitHub 链接
- **参数 / 枚举 / 配置总表**：所有可调 `Application.get_env` / env vars / 权重 / 阈值列齐
- **丢弃 / 失败 / 兜底路径总表**：如果讲解的是可靠性相关（队列、投递、持久化等）**必须有**
- **与设计决策 / 协议 / 相关文档的对应关系**：文末一节，链接回本 AGENTS 引用或 design/*.md
- 中文；可含图表（Mermaid 优先）或外链。

> 直接把本次对话中输出的讲解复制到 Markdown 即可；推荐整理成表格、清单、分节，避免只保留纯文本回复。

### 2.3 索引同步（硬约束）

新文档创建后，**必须同步挂到以下至少两处索引**（覆盖从 `docs/README.md` 进的读者 + 从实现目录进的读者）：

1. **`docs/implementation/elixir/README.md`**：在对应分类表格（"核心路径" / "基础设施" / "边缘模块" 等）加一行链接；
2. **`docs/README.md`（若存在总索引）** / **`docs/module-map.md`**：若该主题覆盖了模块能力边界，也在 module-map 对应功能行加入"实现讲解"链接。

### 2.4 与「文档一致性」的关系

- 本规则**是补充**，不是替代"文档/注释/代码三者语义一致"：
  - 代码改了 → 按 doc-sync-checklist 改设计与实现文档；
  - 用户问了讲解 → 生成讲解文档；
  - 如果讲解文档内容涉及与现有设计/实现文档的差异，**优先改对应 design/*.md，而不是只改讲解文档**。
- 讲解文档的作用是"对已有文档 + 源码的详细解说串读"，不是重新定义协议/语义。
...
```
步骤 1：在对话中给出完整讲解（当前对话，用户立刻可读）
步骤 2：按 §2.1 选目录 + 命名，新建讲解 .md
步骤 3：按 §2.2 把讲解内容结构化写入（含行号链接）
步骤 4：按 §2.3 同步至少两处索引（implementation/README.md + module-map.md 或 docs/README.md）
步骤 5：告知用户已落到哪个文件、挂到了哪些索引
```

**禁止**：只在对话中回答而不写文档。禁止："下一次再补"——必须本次完成。

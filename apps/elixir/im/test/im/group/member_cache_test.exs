defmodule IM.Group.MemberCacheTest do
  ...
  setup do
    ...
    peer = AuthFixtures.create_user!(app_key: owner.app_key, user_id: "mc_p_#{System.unique_integer([:positive])}")
  ...
  test "member? 与 list_member_ids 写穿", %{peer: peer, group: group, ctx: ctx} do
    ...
    assert {:ok, _} = Group.leave(%Pb.Im.Protocol.GroupOperateReq{group_id: group.group_id}, leave_ctx)

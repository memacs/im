defmodule IM.WebSocket.Commands.Group.Update do
  @moduledoc "`CMD_GROUP_UPDATE_REQ`。"
  ...
  alias Pb.Im.Protocol.{CmdType, GroupUpdateReq, Packet}

  def handle(%Packet{} = packet, %ConnectionState{context: ctx} = state) when not is_nil(ctx) do
    with {:ok, %GroupUpdateReq{} = req} <- Codec.decode_payload(packet, GroupUpdateReq),
         {:ok, result} <- Group.update(req, ctx) do
      Helpers.reply_push_and_broadcast(packet, state, result)
    else
      {:error, %Error{} = err} ->
        Helpers.reply_error(packet, state, err, CmdType.value(:CMD_GROUP_UPDATE_REQ))
    end
  end

defmodule IM.WebSocket.Commands.Group.SetAdmin do
  @moduledoc "`CMD_GROUP_SET_ADMIN_REQ`。"
  ...
  def handle(%Packet{} = packet, %ConnectionState{context: ctx} = state) when not is_nil(ctx) do
    with {:ok, %GroupAdminReq{} = req} <- Codec.decode_payload(packet, GroupAdminReq),
         {:ok, result} <- Group.set_admin(req, ctx) do
      Helpers.reply_push_and_broadcast(packet, state, result)
    else
      {:error, %Error{} = err} ->
        Helpers.reply_error(packet, state, err, CmdType.value(:CMD_GROUP_SET_ADMIN_REQ))
    end
  end

defmodule IM.WebSocket.Commands.Group.Kick do
  @moduledoc "`CMD_GROUP_KICK_REQ`。"
  ...
  alias Pb.Im.Protocol.{CmdType, GroupKickReq, Packet}

  def handle(%Packet{} = packet, %ConnectionState{context: ctx} = state) when not is_nil(ctx) do
    with {:ok, %GroupKickReq{} = req} <- Codec.decode_payload(packet, GroupKickReq),
         {:ok, result} <- Group.kick(req, ctx) do
      Helpers.reply_push_and_broadcast(packet, state, result)
    else
      {:error, %Error{} = err} ->
        Helpers.reply_error(packet, state, err, CmdType.value(:CMD_GROUP_KICK_REQ))
    end
  end

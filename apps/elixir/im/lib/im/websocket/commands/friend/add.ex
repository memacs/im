defmodule IM.WebSocket.Commands.Friend.Add do
  @moduledoc "`CMD_FRIEND_ADD_REQ`。"

  alias IM.WebSocket.Commands.Friend.Helpers
  alias IM.WebSocket.ConnectionState
  alias Pb.Im.Protocol.{CmdType, Packet, FriendAddReq}

  def handle(%Packet{} = packet, %ConnectionState{context: ctx} = state) when not is_nil(ctx) do
    Helpers.handle_op(packet, state, FriendAddReq, CmdType.value(:CMD_FRIEND_ADD_REQ))
  end

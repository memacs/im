defmodule IM.WebSocket.Commands.Friend.Block do
  @moduledoc "`CMD_FRIEND_BLOCK_REQ`。"

  alias IM.WebSocket.Commands.Friend.Helpers
  alias IM.WebSocket.ConnectionState
  alias Pb.Im.Protocol.{CmdType, Packet, FriendBlockReq}

  def handle(%Packet{} = packet, %ConnectionState{context: ctx} = state) when not is_nil(ctx) do
    Helpers.handle_op(packet, state, FriendBlockReq, CmdType.value(:CMD_FRIEND_BLOCK_REQ))
  end
